...
DEMO_DB = 'demo_data_v2.db' # <--- Pointing to the V2 file
DB_NAME = '' 
...
hierarchy_df = get_meter_hierarchy();st.sidebar.title("Meter Selector");st.sidebar.markdown("Select meters to analyze:");selected_meters = []
...
if not selected_meters:
    st.warning("Please select at least one meter from the sidebar to view analytics.");st.stop()
tab_overview, tab_analytics, tab_detail, tab_raw_data = st.tabs(
    ["Overview", "Historical Analytics", "Detailed Day Analysis", "Raw Data"]
)
with tab_overview:
    st.header("Consumption Overview");kpi_data = get_kpi_metrics(selected_meters);kpi_cols = st.columns(3);kpi_cols[0].metric("Today's Consumption", f"{kpi_data.get('today_kwh', 0):.2f} kWh");kpi_cols[1].metric("This Week's Total", f"{kpi_data.get('week_kwh', 0):.2f} kWh");kpi_cols[2].metric("This Month's Total", f"{kpi_data.get('month_kwh', 0):.2f} kWh");st.markdown("---");st.header("Live Status");latest_df = get_latest_readings(selected_meters);
    if not latest_df.empty:
        cols = st.columns(len(latest_df));
        for i, (index, row) in enumerate(latest_df.iterrows()):
            with cols[i]:
                st.metric(label=f"{row['lab_name']} Power", value=f"{row['power']:.2f} W");st.caption(f"PF: {row['pf']:.2f} | V: {row['voltage']:.1f} V");st.caption(f"Last read: {row['Last Reading']}")
    st.markdown("---");st.header("Total Consumption Breakdown");total_kwh_df = get_total_consumption_by_meter(selected_meters);
    if not total_kwh_df.empty:
        fig = px.pie(total_kwh_df, names='lab_name', values='Total Consumption (kWh)', title="Total Energy Consumption (kWh) by Meter");st.plotly_chart(fig, use_container_width=True)
with tab_analytics:
    st.header("Consumption Analytics");st.subheader("Daily Energy Usage History (All Selected Meters)");daily_history_df = get_daily_usage_history(selected_meters);
    if not daily_history_df.empty:
        daily_history_df['Date'] = pd.to_datetime(daily_history_df['Date']);fig_daily_history = px.bar(daily_history_df, x="Date", y="Total Units Consumed (kWh)", title="Total Energy Per Day");st.plotly_chart(fig_daily_history, use_container_width=True)
    st.markdown("---");st.subheader("Which day of the week consumes the most energy?");weekday_df = get_consumption_by_weekday(selected_meters);
    if not weekday_df.empty:
        fig_weekday = px.bar(weekday_df, x="Day of Week", y="Total (kWh)", color="lab_name", barmode="group", title="Total Consumption by Day of Week", category_orders={"Day of Week": ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"]});st.plotly_chart(fig_weekday, use_container_width=True)
    st.markdown("---");st.subheader("What time of day are labs most active?");hourly_df = get_consumption_by_hour(selected_meters);
